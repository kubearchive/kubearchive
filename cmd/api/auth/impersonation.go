...
func newImpersonatedData(c *gin.Context) (*impersonatedData, error) {
	...
	if !hasUser {
		if hasGroups || hasUID || hasExtras {
			return nil, errors.New("impersonated user header user required for impersonation")
		}
...
func parseUser(headers http.Header) (*apiAuthzv1.ResourceAttributes, string, string) {
	...
	namespace, sa, err := serviceaccount.SplitUsername(impersonatedUser)
...
func Impersonation(
	...
	return func(c *gin.Context) {
		...
		if imperErr != nil && !errors.Is(imperErr, ErrNoImpersonationData) {
			abort.Abort(c, fmt.Errorf("invalid impersonation data"), http.StatusBadRequest)
			return
		}
