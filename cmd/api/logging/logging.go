...
func LogRetrieval(loggingConfig map[string]string) gin.HandlerFunc {
	...
	if user == "" || password == "" || err != nil {
		return func(c *gin.Context) {
			abort.Abort(c, "invalid jsonPath expression in logging config: "+err.Error(), http.StatusBadRequest)
		}
	}
	return func(c *gin.Context) {
		...
		if errReq != nil {
			abort.Abort(c, errReq.Error(), http.StatusBadRequest)
		}
		...
		if errReq != nil {
			abort.Abort(c, errReq.Error(), http.StatusInternalServerError)
			return
		}
		defer func(Body io.ReadCloser) {
			readErr := Body.Close()
			if readErr != nil {
				abort.Abort(c, readErr.Error(), http.StatusInternalServerError)
				return
			}
		}(response.Body)
		...
		for readErr == nil {
			...
			if errJson != nil {
				abort.Abort(c, "retrieved log line isn't in the expected JSON format: "+errJson.Error(), http.StatusInternalServerError)
				return
			}
		...
		if !logsReturned {
			abort.Abort(c, "no parsed logs for the json path: "+jsonPathParser.String(), http.StatusNotFound)
		}
	}
}
