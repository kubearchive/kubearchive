...
func (c *Controller) GetLogURL(context *gin.Context) {
	...
	if name != "" {
		logURL, jsonPath, err = c.Database.QueryLogURL(
			context.Request.Context(), kind, apiVersion, namespace, name, containerName, false)
	} else {
		logURL, jsonPath, err = c.Database.QueryLogURL(
			context.Request.Context(), kind, apiVersion, namespace, uid, containerName, true)
	}
...
func (c *Controller) GetResourceByUID(context *gin.Context) {
	_, err := discovery.GetAPIResourceKind(context) // Not used but required for validation
	if err != nil {
		abort.Abort(context, err, http.StatusInternalServerError)
		return
	}
	...
	resource, err := c.Database.QueryResourceByUID(context.Request.Context(), uid)
	if err != nil {
		abort.Abort(context, err, http.StatusInternalServerError)
		return
	}
