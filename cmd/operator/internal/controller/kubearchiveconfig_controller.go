...
func (r *KubeArchiveConfigReconciler) reconcileClusterRoleBinding(ctx context.Context, name string, kind string, add bool, subjects ...rbacv1.Subject) (*rbacv1.ClusterRoleBinding, error) {
	...
	slog.Info("in reconcileClusterRoleBinding", "name", name)
	...
	if errors.IsNotFound(err) {
		err = r.Client.Create(ctx, desired)
		if err != nil {
			slog.Error("Failed to create ClusterRoleBinding", "error", err, "name", name)
			return nil, err
		}
		return desired, nil
	} else if err != nil {
		slog.Error("Failed to reconcile ClusterRoleBinding", "error", err, "name", name)
		return nil, err
	}
	...
	err = r.Client.Update(ctx, existing)
	if err != nil {
		slog.Error("Failed to update ClusterRoleBinding", "error", err, "name", name)
		return nil, err
	}
	return existing, nil
}
