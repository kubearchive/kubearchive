...
func (r *SinkFilterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Extract namespaces by kinds from all namespaces
	namespacesByKinds := r.extractNamespacesByKinds(sinkFilter)

	// Generate watches based on the new resource configuration
...
func (r *SinkFilterReconciler) extractNamespacesByKinds(sinkFilter *kubearchivev1.SinkFilter) map[string]map[string]struct{} {
	...
	for namespaceName, resources := range sinkFilter.Spec.Namespaces {
		for _, resource := range resources {
			...
			// Add this namespace to the map for this kind selector
...
func (r *SinkFilterReconciler) parseKindAndAPIVersionFromKey(key string) (string, string) {
	// Key format is "Kind-APIVersion", so we need to parse it back
...
func (r *SinkFilterReconciler) generateWatches(ctx context.Context, namespacesByKinds map[string]map[string]struct{}) error {
	...
	// Find watches that need to be stopped (exist but not in new namespacesByKinds)
	toStop := r.findWatchesToStop(namespacesByKinds)

	// Find watches that need to be created (in new namespacesByKinds but don't exist)
	toCreate := r.findWatchesToCreate(namespacesByKinds)

	// Find watches that need to be updated (exist and are still needed, but may have different configuration)
	...
	// Update existing watches by updating their namespace configuration
	...
	// Create new watches for each new unique kind
...
func (r *SinkFilterReconciler) findWatchesToUpdate(namespacesByKinds map[string]map[string]struct{}, toStop map[string]struct{}) map[string]struct{} {
	...
	// Find watches that exist, are still needed, but are not being stopped
	for existingKey := range r.watches {
		if _, stillNeeded := namespacesByKinds[existingKey]; stillNeeded {
...
func (r *SinkFilterReconciler) createWatchForGVR(ctx context.Context, key string, gvr schema.GroupVersionResource, namespaces map[string]struct{}) {
	...
	// Generate KindSelector from key
	kind, apiVersion := r.parseKindAndAPIVersionFromKey(key)
	kindSelector := sourcesv1.APIVersionKindSelector{
		Kind:       kind,
...
func (r *SinkFilterReconciler) watchLoop(ctx context.Context, watchInfo *WatchInfo, key string) {
	...
	for {
		...
		watchInterface, err := r.createWatch(ctx, watchInfo.GVR)
		if err != nil {
		...
		watchInfo.WatchInterface = watchInterface
		backoff = time.Second // Reset backoff on successful connection

		log.Info("Started watch", "key", key, "gvr", watchInfo.GVR.String())

		// Process watch events
		r.processWatchEvents(ctx, watchInterface, watchInfo, key)

		// Clean up watch interface
...
func (r *SinkFilterReconciler) createWatch(ctx context.Context, gvr schema.GroupVersionResource) (watch.Interface, error) {
	// Watch all namespaces - we'll filter in the event handler
	watchInterface, err := r.dynamicClient.Resource(gvr).Watch(ctx, metav1.ListOptions{
	...
	for {
		...
		case event, ok := <-resultChan:
			...
			if event.Type == watch.Error {
				...
				// Try to extract error details from the Status object
				if status, ok := event.Object.(*metav1.Status); ok {
					errorMsg = status.Message
					errorCode = status.Code
					errorReason = string(status.Reason)
				} else {
					errorMsg = fmt.Sprintf("unknown error format: %T", event.Object)
				}

				log.Error(nil, "Watch error event received, recreating watch",
					"key", key,
...
func (r *SinkFilterReconciler) handleWatchEvent(ctx context.Context, event watch.Event, watchInfo *WatchInfo) error {
	...
	// Apply predicate filtering using the namespaces from watchInfo
	objNamespace := unstructuredObj.GetNamespace()

	// Check if global namespace ("all namespaces") is configured
	_, globalExists := watchInfo.Namespaces[constants.SinkFilterGlobalNamespace]

	// Check if the object's specific namespace is configured
	...
	// Send watch event to sendCloudEvent for processing
...
func (r *SinkFilterReconciler) sendCloudEvent(ctx context.Context, event watch.Event, watchInfo *WatchInfo) {
	...
	if metadata, ok := resource["metadata"].(map[string]interface{}); ok {
		...
		log.Info("Cloud event sent successfully", "eventType", eventType, "gvr", watchInfo.GVR.String(), "kind", watchInfo.KindSelector.Kind, "name", name, "namespace", namespace)
	} else {
		log.Info("Cloud event sent successfully", "eventType", eventType, "gvr", watchInfo.GVR.String(), "kind", watchInfo.KindSelector.Kind)
	}
}
