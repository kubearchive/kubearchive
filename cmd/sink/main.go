...
func (sink *Sink) writeLogs(ctx context.Context, obj *unstructured.Unstructured) {
	...
	if kind != "pod" {
		slog.Info("Object does not have logs associated with it", "objectID", obj.GetUID(), "kind", kind)
		return
	}
	...
	if err != nil {
		slog.Error("Could build log urls for object", "objectID", obj.GetUID(), "err", err)
		return
	}
	...
	if err != nil {
		...
		slog.Info("Successfully wrote log urls for object to the database", "objectID", obj.GetUID(), "kind", kind)
	}
}
