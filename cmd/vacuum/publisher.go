	...
	for _, item := range list.Items {
		...
		if eventTypeSuffix != "" {
			...
			if ce.IsACK(sendResult) {
				slog.Info("Event sent successfully", "apiversion", avk.APIVersion, "kind", avk.Kind, "namespace", namespace, "name", name, "eventType", eventType)
			} else {
				...
				slog.Info("Event send failed", "apiversion", avk.APIVersion, "kind", avk.Kind, "namespace", namespace, "name", name, "eventType", eventType, "code", statusCode)
			}
		} else {
