...
[IMPORTANT]
====
Automatic service discovery does the following checks:

* **ConfigMap** - Check for the field `URL` under a `ConfigMap` named `kubearchive-api-url`
* **OpenShift Routes** - Services exposed via OpenShift route resources. This piece needs the CLI user to have `list` permissions over the `Routes`
