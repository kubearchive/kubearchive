...
* Definition of interfaces that need to be implementing when adding support to a new database engine.
* Usage of a SQL Builder.

=== Database Interfaces

To add a new database engine the following interfaces must be implemented:

`Database`::
Composed by `DBWriter` and `DBReader` described below.

`DBReader`::
Include the methods that should be used by the KubeArchive API.
Encapsulates the business logic related to the different read database operations:
`QueryResources` and `QueryLogURL` as well as the unexported getters of the implementations interfaces involved in
creating write SQL statements pieces: `DBSelector`, `DBSorter` and `DBFilter`.
It also includes some general database management methods like `Ping` and `CloseDB`.

`DBWriter`::
Include the methods that should be used by the KubeArchive sink.
Encapsulates the business logic related to the different write database operations:
`WriteResources` and `WriteUrls` as well as the unexported getters of the implementations interfaces involved in
creating write SQL statements pieces: `DBInserter`, `DBDeleter` and `DBFilter`.
It also includes some general database management methods like `Ping` and `CloseDB`.
