...
This document explains how to configure KubeArchive so you get resources
archived.
...
To configure KubeArchive to archive or delete resources create a
KubeArchiveConfig custom resource. KubeArchiveConfigs are limited to 1 per namespace
and should be named `kubearchive`. The custom resource has this general form:
...
`spec.resources` is a list of elements, each defining rules so KubeArchive knows
...
The pod should not appear in KubeArchive until it completes. After a while is present
on KubeArchive:
...
The pod keeps existing on Kubernetes, occupying space:
...
They key feature of KubeArchive is deleting resources. This feature keeps
the Kubernetes cluster free of resources that are not needed anymore. To enable
deletion of resources use the `deleteWhen` key.
...
Start watching pods to see how this pod gets created, executed, completed and deleted:
...
== `archiveOnDelete`: Archiving on Deletion

KubeArchive also can be used with existing applications that clean up resources.
...
to archive pods when they get deleted only if they match the condition `status.phase == "Succeeded"`
so failed pods that get deleted do not get archived.
...
Query KubeArchvie to check that only the pod that completed correctly (`archived-on-deletion`)
was archived:
...
to resource archiving. Explore this documentation to learn more
