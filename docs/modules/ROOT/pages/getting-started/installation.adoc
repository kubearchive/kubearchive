...
* CertManager is installed on the Kubernetes cluster (+v1.19.3)

== Prepare the Database

. Create a user and a password for KubeArchive to use.
. Create a database for KubeArchive.
  Follow link:https://github.com/kubearchive/kubearchive/blob/{page-component-display-version}/integrations/database/postgresql/setup.sql[KubeArchive's setup.sql]
  for options that need to be applied to the database.
. Clone the
...
There are a couple of ways to install KubeArchive:

=== Standalone KubeArchive

This installation method directly installs KubeArchive in the `kubearchive`
namespace.
