...
-- Create label_pair table to store unique key-value combinations
CREATE TABLE public.label_pair (
    id BIGSERIAL PRIMARY KEY,
    ...
    CONSTRAINT unique_label_pair UNIQUE (key_id, value_id)
);
...
CREATE TABLE public.resource_label (
    ...
    pair_id BIGINT NOT NULL REFERENCES label_pair(id) ON DELETE CASCADE,
...
REVOKE UPDATE ON public.label_pair FROM PUBLIC;
...
-- Create resource-label associations using pair_id
