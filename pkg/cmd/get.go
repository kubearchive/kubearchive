...
// maxPaginationPages caps how many KubeArchive pages a single get will fetch.
// It guards against looping forever if the server keeps returning a non-empty
// continue token with empty (or endlessly repeating) pages. It is a var so
// tests can lower it. 10000 pages at the 1000-item server max covers 10M
// resources, well beyond any realistic interactive query.
...
func (o *GetOptions) buildKubeArchiveQueryParams() url.Values {
	...
	if o.Count {
		...
		params.Set("limit", fmt.Sprintf("%d", o.effectivePageSize()))
	}
	if !o.After.IsZero() {
...
// pageSize computes the effective per-request page size from the --page-size and
// --limit flags. It is a pure computation; runtime callers should use
// effectivePageSize, which resolves the value once and caches it.
...
// effectivePageSize returns the per-request page size, resolving it from the
// flags on first use and caching the result so it is not re-derived on every
// request. pageSize never returns 0, so a zero cache means "not yet resolved".
func (o *GetOptions) effectivePageSize() int {
	if o.resolvedPageSize == 0 {
		o.resolvedPageSize = o.pageSize()
	}
	return o.resolvedPageSize
}
...
//
// Both init and resources must be sorted newest-first by creation timestamp,
// matching the order cmpResource expects. Passing oldest-first data causes the
// binary search to compute wrong insertion points and corrupt the ordering.
...
// The watermark used to settle streamed output is timestamp-only, while
// KubeArchive paginates by the composite key (creationTimestamp, DB id). If a
// page boundary splits a group of resources that share the same
// creationTimestamp, those tied resources can be emitted across separate batches.
// In that rare case ordering within the tie group may be slightly off, and an
// in-cluster resource whose archived copy lands on a later page may not be
// deduplicated. This is a known, accepted limitation; handling it would require
// buffering the entire oldest-timestamp tie group across pages.
...
type resourceStreamer interface {
	// emitBatch writes resources up to the streamer's limit, returning the
	// number actually written. It returns an error if a resource cannot be
	// marshaled so the caller can fail instead of producing truncated output.
	emitBatch(resources []*ResourceWithAvailability) (int, error)
	close() error
	count() int
	oldest() time.Time
}
...
func (ts *tableStreamer) emitBatch(resources []*ResourceWithAvailability) (int, error) {
	if len(resources) == 0 {
		return 0, nil
	}
...
func (js *jsonStreamer) emitBatch(resources []*ResourceWithAvailability) (int, error) {
	emitted := 0
	for _, rwa := range resources {
		...
		// Marshal before writing any separators so a failure cannot leave a
		// dangling comma and produce malformed JSON.
		itemBytes, err := json.MarshalIndent(rwa.Resource.Object, "        ", "    ")
		if err != nil {
			return emitted, fmt.Errorf("error marshaling resource %s to JSON: %w", rwa.Resource.GetName(), err)
		}
...
func (ys *yamlStreamer) emitBatch(resources []*ResourceWithAvailability) (int, error) {
	emitted := 0
	for _, rwa := range resources {
		...
		// Marshal before writing anything so a failure cannot emit a partial
		// list item.
		itemBytes, err := yaml.Marshal(rwa.Resource.Object)
		if err != nil {
			return emitted, fmt.Errorf("error marshaling resource %s to YAML: %w", rwa.Resource.GetName(), err)
		}
...
func (o *GetOptions) runStreaming(k8sResources []*unstructured.Unstructured, k8sNotFound bool) error {
	...
	if o.Archived {
		...
		for {
			if pagesFetched >= maxPaginationPages {
				fmt.Fprintf(o.ErrOut, "\nWarning: stopped after %d pages to avoid an unbounded pagination loop; results may be incomplete.\n", maxPaginationPages)
				moreArchived = true
				break
			}
			pagesFetched++
			...
			emitted, emitErr := streamer.emitBatch(batch)
			if emitErr != nil {
				return emitErr
			}
			...
			if o.Limit > 0 {
				...
				queryParams.Set("limit", fmt.Sprintf("%d", min(rem, o.effectivePageSize())))
			}
		}
	}
	...
	if len(remaining) > 0 {
		...
		emitted, emitErr := streamer.emitBatch(finalBatch)
		if emitErr != nil {
			return emitErr
		}
