...
const (
	...
	version                  = "v1"
	namespace                = "cpaas-ci"
	queryResources           = "SELECT data FROM test WHERE kind=\\$1 AND api_version=\\$2"
	queryNamespacedResources = "SELECT data FROM test WHERE kind=\\$1 AND api_version=\\$2 AND namespace=\\$3"
...
func TestQueryResources(t *testing.T) {
	...
	mock.ExpectQuery(queryResources).WithArgs(kind, apiVersion).WillReturnRows(rows)
...
func TestQueryNamespacedResources(t *testing.T) {
	...
	mock.ExpectQuery(queryNamespacedResources).WithArgs(kind, apiVersion, namespace).WillReturnRows(rows)

	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()

	resources, err := database.QueryNamespacedResources(ctx, kind, group, version, namespace)
	assert.NotNil(t, resources)
	assert.NotEqual(t, 0, len(resources))
	assert.NoError(t, err)
}
