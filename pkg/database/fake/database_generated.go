...
func (f *fakeDatabase) QueryLogURL(_ context.Context, _, _, _, _, _ string, _ bool) (string, string, error) {
	if len(f.logUrl) == 0 {
		return "", "", f.err
	}
...
func (f *fakeDatabase) QueryResourceByUID(ctx context.Context, uid string) (*models.Resource, error) {
