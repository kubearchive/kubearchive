...
func (db *Database) Init(env map[string]string) error {
...
func (db *Database) QueryResources(ctx context.Context, kind, apiVersion, namespace, name,
	continueId, continueDate string, labelFilters *database.LabelFilters, limit int) ([]string, int64, string, error) {
...
func (db *Database) getLogsForPodSelector(ctx context.Context, sb *sqlbuilder.SelectBuilder, namespace, name string) (string, string, error) {
	...
	if len(podString) == 0 {
		return "", "", database.ResourceNotFoundError
	}
	...
	containerName, ok := annotations[database.DefaultContainerAnnotation]
	if !ok {
