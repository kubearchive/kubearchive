...
// Helper methods for building positive filter conditions using EXISTS subqueries

func (postgreSQLFilter) applyExistsFilter(sb *sqlbuilder.SelectBuilder, keys []string) {
	for _, key := range slices.Sorted(slices.Values(keys)) {
		existsSubquery := fmt.Sprintf(
			"EXISTS (SELECT 1 FROM resource_label rl "+
				"JOIN label_key_value lp ON lp.id = rl.pair_id "+
				"JOIN label_key lk ON lk.id = lp.key_id "+
				"WHERE rl.resource_id = resource.id AND lk.key = %s)",
			sb.Var(key),
		)
...
func (postgreSQLFilter) applyEqualsFilter(sb *sqlbuilder.SelectBuilder, labels map[string]string) {
	for _, key := range slices.Sorted(maps.Keys(labels)) {
		...
		existsSubquery := fmt.Sprintf(
			"EXISTS (SELECT 1 FROM resource_label rl "+
				...
				"WHERE rl.resource_id = resource.id AND lk.key = %s AND lv.value = %s)",
			sb.Var(key),
			sb.Var(value),
		)
		sb.Where(existsSubquery)
...
func (postgreSQLFilter) applyInFilter(sb *sqlbuilder.SelectBuilder, labels map[string][]string) {
	for _, key := range slices.Sorted(maps.Keys(labels)) {
		values := labels[key]
		...
		existsSubquery := fmt.Sprintf(
			"EXISTS (SELECT 1 FROM resource_label rl "+
				...
				"JOIN label_key lk ON lk.id = lp.key_id "+
				"JOIN label_value lv ON lv.id = lp.value_id "+
				"WHERE rl.resource_id = resource.id AND lk.key = %s AND lv.value IN (%s))",
			sb.Var(key),
...
func (f postgreSQLFilter) ApplyLabelFilters(sb *sqlbuilder.SelectBuilder, labelFilters *models.LabelFilters) {
	if len(labelFilters.Exists) > 0 {
		f.applyExistsFilter(sb, labelFilters.Exists)
	}
	if len(labelFilters.Equals) > 0 {
		f.applyEqualsFilter(sb, labelFilters.Equals)
	}
	if len(labelFilters.In) > 0 {
		f.applyInFilter(sb, labelFilters.In)
	}
