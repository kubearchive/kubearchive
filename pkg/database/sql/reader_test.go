			...
			for _, ttt := range subtests {
				t.Run(ttt.name, func(t *testing.T) {
					...
					if ttt.numResources == 0 {
						...
						assert.Equal(t, int64(5), resources[0].Id)
					}
...
// The expectations match the queries executed by resolveKeyIDs, resolveLabelIDs, and
// resolveLabelIDsMulti in the order they are called from ApplyLabelFilters.
func setupLabelResolutionMocks(mock sqlmock.Sqlmock, labelFilters *models.LabelFilters) {
	...
	if len(allKeys) > 0 {
		...
		sb.Select("id", "key").From("label_key")
		keyValues := make([]interface{}, len(keySlice))
		for i, k := range keySlice {
			keyValues[i] = k
		}
		sb.Where(sb.In("key", keyValues...))
		query, _ := sb.BuildWithFlavor(sqlbuilder.PostgreSQL)
	...
	// resolveLabelIDs - needed for Equals, NotEquals
	...
	if len(allEqualsPairs) > 0 {
		...
		sb := sqlbuilder.NewSelectBuilder()
		sb.Select("lkv.id", "lk.key", "lv.value")
		sb.From("label_key_value lkv")
		sb.Join("label_key lk", "lk.id = lkv.key_id")
		sb.Join("label_value lv", "lv.id = lkv.value_id")
		orConditions := make([]string, 0, len(keys))
		for _, key := range keys {
			orConditions = append(orConditions, sb.And(sb.Equal("lk.key", key), sb.Equal("lv.value", allEqualsPairs[key])))
		}
	...
	// resolveLabelIDsMulti - needed for In, NotIn
	...
	if len(allMultiPairs) > 0 {
		...
		for _, key := range keys {
			for _, value := range allMultiPairs[key] {
				orConditions = append(orConditions, sb.And(sb.Equal("lk.key", key), sb.Equal("lv.value", value)))
			}
		}
		sb.Where(sb.Or(orConditions...))
		query, _ := sb.BuildWithFlavor(sqlbuilder.PostgreSQL)
