...
func TestLogURLsFromNonExistentResource(t *testing.T) {
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			logUrl, jp, err := tt.database.QueryLogURL(ctx, kind, cronJobApiVersion, namespace, cronJobName, "", false)
...
func TestCronJobQueryLogURLsByUID(t *testing.T) {
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sb.Where(filter.UuidFilter(sb.Cond, cronJobUID))
			...
			logUrl, jp, err := tt.database.QueryLogURL(ctx, kind, cronJobApiVersion, namespace, cronJobUID, "", true)
	...
	for _, tt := range tests {
		for _, innerTest := range innerTests {
			t.Run(innerTest.name, func(t *testing.T) {
				...
				logUrl, jp, err := tt.database.QueryLogURL(context.Background(), pod.Kind, pod.APIVersion, pod.Namespace, pod.Name, "", false)
				assert.NoError(t, err)
...
func TestQueryResourceByUID(t *testing.T) {
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			filter := tt.database.getFilter()
			sb := tt.database.getSelector().ResourceSelector()
			sb.Where(filter.UuidFilter(sb.Cond, podUUID))
			query, _ := sb.BuildWithFlavor(tt.database.getFlavor())
			...
			for _, ttt := range subtests {
				t.Run(ttt.name, func(t *testing.T) {
					...
					mock.ExpectQuery(regexp.QuoteMeta(query)).WithArgs(podUUID).WillReturnRows(rows)

					ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
					defer cancel()

					resource, err := tt.database.QueryResourceByUID(ctx, podUUID)
					if ttt.numResources == 0 {
