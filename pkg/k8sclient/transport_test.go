...
func TestRoundTrip(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Append tc.path to the server's base URL (e.g. http://127.0.0.1:PORT/api/v1/nodes)
			...
			if routeVal != tc.wantRoute {
				t.Errorf("route attribute = %q, want %q", routeVal, tc.wantRoute)
			}
		})
	}
}
