...
func TestClusterVacuum(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"cvac-keep-last-when-both-configs-overlapping": {
			expected:  "testdata/cvac-keep-last-when-both-overlap.txt",
			ckac:      "testdata/ckac-keep-last-when-overlap.yaml",
			kac:       "testdata/kac-keep-last-when-overlap.yaml",
			vacuumRes: []resourceType{{filename: "testdata/vac-job-resource.yaml", addNS: true}},
			allRes:    "",
		},
