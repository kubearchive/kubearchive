...
func createClusterVacuumConfig(t testing.TB, cvc *kubearchiveapi.ClusterVacuumConfig) {
	t.Log(fmt.Sprintf("Creating cluster vacuum config '%s/%s'", cvc.ObjectMeta.Namespace, cvc.ObjectMeta.Name))
...
func deleteClusterVacuumConfig(t testing.TB) {
	t.Log(fmt.Sprintf("Deleting cluster vacuum config '%s/%s'", constants.KubeArchiveNamespace, clusterVacuumConfigName))
	...
	retryErr := retry.Do(func() error {
		...
		if !errs.IsNotFound(getErr) {
			return errors.New(fmt.Sprintf("Waiting for cluster vacuum config '%s/%s' to be deleted", constants.KubeArchiveNamespace, clusterVacuumConfigName))
		}
		return nil
...
func deleteClusterVacuumJob(t testing.TB, name string) {
	t.Log(fmt.Sprintf("Deleting cluster vacuum job '%s/%s'", constants.KubeArchiveNamespace, name))
	...
	retryErr := retry.Do(func() error {
		...
		if !errs.IsNotFound(getErr) {
			return errors.New(fmt.Sprintf("Waiting for cluster vacuum job '%s/%s' to be deleted", constants.KubeArchiveNamespace, name))
		}
		return nil
	}, retry.Attempts(10), retry.MaxDelay(3*time.Second))
...
func newClusterVacuumConfig(spec kubearchiveapi.ClusterVacuumConfigSpec) *kubearchiveapi.ClusterVacuumConfig {
	return &kubearchiveapi.ClusterVacuumConfig{
					...
					Containers: []corev1.Container{
						{
							...
							Image:   "quay.io/gallen/vacuum:latest",
							Env: []corev1.EnvVar{
								{
