// Copyrighiit KubeArchive Authors
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if auth != tt.expectedAuth {
				t.Fatalf("Expected unauth to be %v but got %v", tt.expectedAuth, auth)
			}
		})
	}
