...
func TestDefaultContainer(t *testing.T) {
	...
	pods := map[string]map[string]string{
		"defaults-to-first": map[string]string{},
		"wants-second": map[string]string{
			"kubectl.kubernetes.io/default-container": "second",
		},
		"empty-annotation": map[string]string{
			"kubectl.kubernetes.io/default-container": "",
		},
	}

	for podName, annotations := range pods {
		// Create a pod
		pod := &corev1.Pod{
			ObjectMeta: metav1.ObjectMeta{
				Name:        podName,
	...
	expected := map[string]string{
		"defaults-to-first": "I'm the container called first.",
		"wants-second":      "I'm the container called second.",
		"empty-annotation":  "I'm the container called first.",
	}

	for podName, expectedLog := range expected {
		t.Logf("checking logs for pod '%s', expected log '%s'", podName, expectedLog)
		url := fmt.Sprintf("https://localhost:%s/api/v1/namespaces/%s/pods/%s/log", port, namespaceName, podName)
		retryErr := retry.Do(func() error {
			body, err := test.GetLogs(t, token.Status.Token, url)
			if err != nil {
				return err
			...
			if strings.Trim(bodyString, "\n") != expectedLog {
				t.Log("log does not match")
				return fmt.Errorf("log does not match the expected '%s'", expectedLog)
			}
