...
var testMap = map[string]map[string]string{
	"emptyKAC":    {"applyFilters": "0", "deleteFilters": "0"},
	"nonEmptyKAC": {"applyFilters": "1", "deleteFilters": "0"},
}
...
func runTest(t testing.TB, testName string, resources map[string]any) {
	t.Helper()

	namespace, _ := test.CreateTestNamespace(t, false)

	test.CreateKAC(t, namespace, resources)
	checkResourcesAfterApply(t, namespace, testName)
	test.DeleteKAC(t, namespace)
	checkResourcesAfterDelete(t, namespace, testName)
}
