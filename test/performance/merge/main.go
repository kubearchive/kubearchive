...
func main() {
	...
	fmt.Fprintln(fh, "created_at,api_max,api_min,api_mean,api_median,sink_max,sink_min,sink_mean,sink_median,operator_max,operator_min,operator_mean,operator_median")
...
func getStats(wr WorkflowRuns, path, metricType string) []Stats {
	...
	for _, run := range wr.WorkflowRuns {
		if run.Event == "schedule" {
			...
			pkg.ExtractStats(filePath, metricType, "json", &outBuff)

			var stat Stats
			err := json.Unmarshal(outBuff.Bytes(), &stat)
			if err != nil {
				panic(fmt.Sprintf("error deserializing stats output: %s, %s", err.Error(), outBuff.String()))
			}
